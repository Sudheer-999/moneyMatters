...
const LastTransactionItem = (props) => {
  ...
  const showOptions = loginId === "3" ? "hide" : "show";

  const pullRight = loginId === "3" ? "pull" : "";

  const filteredImage = fakeImageData.filter(
    (eachItem) => eachItem.id === listNum.toString()
  );

  const { avatar, name } = filteredImage[0];

  const showImage = loginId === "3" ? "show-image" : "hide-image";
    ...
    <tr className="list-item">
      <td className=" line">
        <div className="trans-name">
          ...
          <div className={` image-block ${showImage}`}>
            <img src={avatar} alt="person" className="person-image" />
          </div>
          <p className="t-name">{nameField}</p>
        </div>
      </td>

      <td className="t-category line">{category}</td>
      <td className="t-date t-category line">{formateDate(dateString)}</td>
      <td className={`t-amount line ${amountColor} `}>
      ...
      <td className="line">
        <div className={`trans-end ${showOptions}`}>
